...
const Hero = () => {
  ...
  return (
    <section ref={sectionRef} className="min-h-screen flex items-center pt-24 pb-16 px-6 relative overflow-hidden">
      <div className="max-w-6xl mx-auto w-full grid md:grid-cols-2 gap-12 md:gap-8 items-center">
        ...
        <div className="relative flex justify-center md:justify-end">
          ...
          {/* Doodles */}
          <StarDoodle className="hero-doodle absolute -top-4 left-4 text-secondary animate-float" size={36} />
          <FlowerDoodle className="hero-doodle absolute bottom-8 -left-4 text-accent animate-wiggle" size={40} />
          <HeartDoodle className="hero-doodle absolute top-12 -right-6 text-accent animate-float-reverse" size={30} />
          <ArrowDoodle className="hero-doodle absolute -bottom-2 right-8 text-foreground rotate-[20deg]" size={90} />
          <StarDoodle className="hero-doodle absolute bottom-0 left-16 text-foreground animate-spin-slow" size={16} />
        </div>
      </div>
    </section>
  );
